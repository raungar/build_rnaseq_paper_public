...
# tissue_type<-"Blood"
# counts_udn_file<-"/oak/stanford/groups/smontgom/shared/UDN/Preprocessing/eOutliers/udn_hg38.tpm.txt.gz"
# counts_control_file<-"/oak/stanford/groups/smontgom/shared/UDN/Preprocessing/eOutliers/dgn_hg38.tpm.txt.gz"
...
counts_control_melt<-fread(counts_control_file)
...
print("counts_control_melt")
print(head(counts_control_melt))
counts_control<-data.table::dcast(counts_control_melt[,c("gene_id","rd_id","TPM")],gene_id~rd_id)
print("head counts control")
print(head(counts_control))
rownames(counts_control)<-counts_control$gene_id
...
rownames(counts_udn)<-c(counts_udn$gene_id)
counts_udn<-counts_udn %>% select("gene_id",metadata_udn$sample_id)
#counts_udn<-counts_udn[,c("gene_id",metadata_udn$sample_id)]

#### merge the counts together
print("check2 -- gene ids of udn then control the combined")
head(counts_udn$gene_id)
head(counts_control$gene_id)
all_counts<-merge(counts_udn,counts_control,by=c("gene_id"))
head(all_counts$gene_id)
...
all_counts_filtered_rns<-as.data.frame(cbind(rownames(all_counts_filtered),all_counts_filtered))
colnames(all_counts_filtered_rns)[1]<-"gene_id"
rownames(all_counts_filtered_rns)<-all_counts_filtered_rns$gene_id
...
print("check4: all_counts_filtered_rns gene id")
head(all_counts_filtered_rns$gene_id)
...
print("check5: temp_dat")  
print(head(temp_dat[1:5,1:5]))
...
saveRDS(sva_fit,"/oak/stanford/groups/smontgom/shared/UDN/Preprocessing/eOutliers/sva_fit_WholeBlood_dgn.RData")
## Splines
################## ADDD IN GTEX BATCH OKAY!!! GREAT :) 
batch <- c(metadata_udn$batch,rep("DGN_BATCH",length(sample_id_control))) #gtex batch: SMNABTCH, center=SMCENTER
study <- c(metadata_udn$institution,rep("SINGLE_INSTUTION",length(sample_id_control)))
...
get_zscore<-function(fitsv){
  ...
  print("WITHING GENE ZSCORE")
  print(head(gene_zscore))
  ...
  # colnames(gene_zscore_num)<-
   print("PRINT ROWNAMES VRVVVV")
   print(head(rownames(gene_zscore_num)))
   print("COLNMAES??")
   print(colnames(gene_zscore_num))
