...
with open(infile,"r") as f_read:
	for line in f_read.readlines():
		...
		transcript_spl=end_of_line.split('source_transcript=')
		gene_spl=end_of_line.split('source_gene=')
		chm13_gene_spl=end_of_line.split('gene_id=')
		if(len(transcript_spl)>1):
			transcript=(transcript_spl[1].split(';')[0])
		if(len(gene_spl)>1):
			gene=(gene_spl[1].split(';')[0])
		if(len(chm13_gene_spl)>1):
			chm13_gene=(chm13_gene_spl[1].split(';')[0])
		...
		#(enst_or_ensg)
		store_line=[chrom,start,end,cluster, sample,z,transcript,gene,chm13_gene,enst_or_ensg]
